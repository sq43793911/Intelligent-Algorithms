function K = Kostberechen(AG,TL,TFK,S,TB,L,LB,BtAF,TA,BsP) %成本计算，ga中的适应性函数
%% 基本思路
...
[NIND, N]= size(BsP);
K = zeros(NIND,1);
minBT = [5 7 10 15 20 30];    %每类的最低订货周期，低于这个周期则会产生缺货
...
for i = 1:NIND
    ...
    %% 计算库存成本
    for f = 1:N  
        e(f) = 255/BsP(i,f);    %每类的订货次数
        %Tag(f) = BsP(i,f)/BtAF(f);
        if BsP(i,f) >=  minBT(f)      %如果不缺货
            c(f) = BsP(i,f)-minBT(f);
                for g = 1:BsP(i,f)
