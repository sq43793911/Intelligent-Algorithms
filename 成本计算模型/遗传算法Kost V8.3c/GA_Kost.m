...
% 修正运输成本，每天缺货成本
%更新历史：
%V8.0c 按照ABC分类法重新分类，此程序计算C类，订货周期为寻优变量，重新修订成本计算模型。 18.11
%V7.1 修改变量名  16.11
%V7.0 将输入变量更改为订货量，修正成本计算模型，修正计算参数。  15.11
...
LB = [0.01 0.03 0.06 0.09 1.2 1.5];   %存储成本,每类的每天存储费：元/个
N = 6;             %N为订购分组
TB = [200 300 400 700 1100 1500];  %运输成本
VST = 5;        %安全提前期，满足该参数天数生产的需要
minni = [5 7 10 15 20 30];    %每类的最低订货周期，低于这个周期则会产生缺货
...
BsP = zeros(N,1);
for x = 1:N
    BtAF(x) = (f(x)*6)/15;    %每类的每天需求
    TA(x)=f(x)*6*102;       %计算每类的总需求
...
BsM = zeros(1,N);
for i = 1:N
    BsM(i) = Best(i)*BtAF(i);    %每类订货量
end
% H = [15 15 20 20 30 30];      %原来的订货量
