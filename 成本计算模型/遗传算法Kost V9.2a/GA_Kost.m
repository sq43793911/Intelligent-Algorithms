...
%更新历史：
%V8.2a 修正运输成本，缺货成本
%V8.0a 按照ABC分类法重新分类，此程序计算A类，使用订货量为变量
%V7.1 修改变量名
...
LB = [0.08 1 5 10 15];   %存储成本,每类的每天存储费：元/个
N = 8;             %N为订购分组
TB = [6000 8000 10000 15000 20000];  %运输成本
VST = 5;        %安全提前期，满足该参数天数生产的需要
minni = [30 45 60 120 160];    %每类的最低订货周期，低于这个周期则会产生缺货
...
BsP = zeros(1,N);
for x = 1:N
    BtAF(x) = (BOMA(x,6)*6)/15;    %每类的每天需求
    TA(x)=BOMA(x,6)*6*102;       %计算每类的总需求
...
%% 优化
gen=0;
ObjV = Kostberechen(AG,TL,TFK,S,TB,L,LB,BtAF,TA,BOMA,Chrom);   %计算成本
preObjV=min(ObjV);
for gen=1:MAXGEN
    %% 计算适应度
    ObjV = Kostberechen(AG,TL,TFK,S,TB,L,LB,BtAF,TA,BOMA,Chrom);  %计算成本
