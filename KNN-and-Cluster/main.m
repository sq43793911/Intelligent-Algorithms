%% 警力资源处理
% 基于事故数据，路网数据，警力数据的警力动态调度寻优
% 采用粒子群算法进行寻优，后期可能调整
% 输入变量为警力和事故的对应关系，在主程序中为
% 
...
%% 导入数据
% 导入事故数据
[Acc_info,Acc_txt] = xlsread ('20180416-0701-0802hefei incident');      

% 导入路网数据
% 路口坐标为第11、12列
R_info = xlsread ('20180412roadnetwork');     

% 导入警力数据
P_info = xlsread ('');

[acc_size_x, acc_size_y] = size(Acc_info);

[r_size_x, r_size_y] = size(R_info);

[p_size_x, p_size_y] = size(P_info);
...
%% 数据预处理
% 判断警力数和事故数的关系，决定种群的编码类型

% 路口坐标
cross_gps = cross_init(R_info);

% 事故相关时间处理
% 将时间格式转换为matlab的数字格式
acc_time = time2num(Acc_txt);
